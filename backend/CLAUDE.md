...
- **Lightpanda MCP v1.0**: TypeScript MCP server in `mcp-tools/lightpanda-mcp/` providing 12 browser tools (goto, search, markdown, links, click, execute_js, fill_form, extract_data, fetch_api, wait_for, get_text, screenshot) via HTTP transport. Connects to Lightpanda browser via CDP WebSocket. Runs as Docker sidecar (`http://lightpanda-mcp:3000/mcp`). URL configured via `$LIGHTPANDA_MCP_URL` env var. Legacy gomcp (stdio-based) still available via `scripts/run-gomcp.sh` but deprecated in favor of the HTTP MCP server.
