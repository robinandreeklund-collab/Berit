...
/**
 * Riksbank MCP HTTP Server — Express-based Streamable HTTP MCP transport.
 *
 * Endpoints:
...
import { RiksbankMCPServer } from './index.js';
...
app.get('/mcp', (req, res) => {
  res.json({
    ...
    description: 'Ekonomisk data från Sveriges Riksbank — 8 verktyg',
    authentication: 'none',
    transport: 'http',
    capabilities: {
      tools: true,
    ...
    if (method === 'initialize') {
      return res.status(200).json({
        ...
        result: {
          ...
          serverInfo: { name: 'Riksbank MCP Server', version: '1.0.0' },
        },
      });
    }
...
<head>
  ...
  <title>Riksbank MCP Server — Dokumentation</title>
  <style>
...
// ── Start ───────────────────────────────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`Riksbank MCP HTTP Server running on port ${PORT}`);
