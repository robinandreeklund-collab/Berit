...
/**
 * Trafikverket MCP HTTP Server — Express-based Streamable HTTP MCP transport.
 *
 * Endpoints:
...
import { TrafikverketMCPServer } from './index.js';
...
app.get('/mcp', (req, res) => {
  res.json({
    ...
    description: 'Realtids trafikdata från Trafikverkets Open API — 22 verktyg',
    authentication: 'none',
    transport: 'http',
    capabilities: {
      tools: true,
    ...
    if (method === 'initialize') {
      return res.status(200).json({
        ...
        result: {
          ...
          serverInfo: { name: 'Trafikverket MCP Server', version: '1.0.0' },
        },
      });
    }
...
<head>
  ...
  <title>Trafikverket MCP Server — Dokumentation</title>
  <style>
...
// ── Start ───────────────────────────────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`Trafikverket MCP HTTP Server running on port ${PORT}`);
