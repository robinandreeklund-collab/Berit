...
/**
 * Trafikanalys MCP HTTP Server — Express-based Streamable HTTP MCP transport.
 *
 * Endpoints:
...
import { TrafikanalysMCPServer } from './index.js';
...
app.get('/mcp', (req, res) => {
  res.json({
    ...
    description: 'Svensk transportstatistik från Trafikanalys (trafa.se) — 8 verktyg',
    authentication: 'none',
    transport: 'http',
    capabilities: {
      tools: true,
    ...
    if (method === 'initialize') {
      return res.status(200).json({
        ...
        result: {
          ...
          serverInfo: { name: 'Trafikanalys MCP Server', version: '1.0.0' },
        },
      });
    }
...
<head>
  ...
  <title>Trafikanalys MCP Server — Dokumentation</title>
  <style>
...
// ── Start ───────────────────────────────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`Trafikanalys MCP HTTP Server running on port ${PORT}`);
