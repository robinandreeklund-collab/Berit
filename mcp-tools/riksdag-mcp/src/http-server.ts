...
/**
 * Riksdag MCP HTTP Server — Express-based Streamable HTTP MCP transport.
 *
 * Endpoints:
...
import { RiksdagMCPServer } from './index.js';
...
app.get('/mcp', (req, res) => {
  res.json({
    ...
    description: 'Data från Sveriges Riksdag & Regeringskansliet — 15 verktyg',
    authentication: 'none',
    transport: 'http',
    capabilities: {
      tools: true,
    ...
    if (method === 'initialize') {
      return res.status(200).json({
        ...
        result: {
          ...
          serverInfo: { name: 'Riksdag MCP Server', version: '1.0.0' },
        },
      });
    }
...
<head>
  ...
  <title>Riksdag MCP Server — Dokumentation</title>
  <style>
...
// ── Start ───────────────────────────────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`Riksdag MCP HTTP Server running on port ${PORT}`);
