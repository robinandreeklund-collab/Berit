...
/**
 * Polisen MCP HTTP Server — Express-based Streamable HTTP MCP transport.
 */
...
import { PolisenMCPServer } from './index.js';
...
const mcpServer = new PolisenMCPServer();
...
app.get('/mcp', (req, res) => {
  res.json({
    protocol: 'mcp',
    version: '1.0.0',
    name: 'Polisen MCP Server',
    description: 'Polishändelser i Sverige — 1 verktyg',
    authentication: 'none',
    transport: 'http',
    ...
    if (method === 'initialize') {
      return res.status(200).json({
        ...
        result: {
          ...
          serverInfo: { name: 'Polisen MCP Server', version: '1.0.0' },
        },
      });
    }
...
<head>
  ...
  <title>Polisen MCP Server — Dokumentation</title>
  <style>
...
app.listen(PORT, () => {
  console.log(`Polisen MCP HTTP Server running on port ${PORT}`);
