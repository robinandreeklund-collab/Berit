...
/**
 * Kolada MCP HTTP Server — Express-based Streamable HTTP MCP transport.
 *
 * Endpoints:
...
import { KoladaMCPServer } from './index.js';
...
app.get('/mcp', (req, res) => {
  res.json({
    ...
    description: 'Svensk kommunstatistik från Kolada (RKA) — 10 verktyg',
    authentication: 'none',
    transport: 'http',
    capabilities: {
      tools: true,
    ...
    if (method === 'initialize') {
      return res.status(200).json({
        ...
        result: {
          ...
          serverInfo: { name: 'Kolada MCP Server', version: '1.0.0' },
        },
      });
    }
...
<head>
  ...
  <title>Kolada MCP Server — Dokumentation</title>
  <style>
...
// ── Start ───────────────────────────────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`Kolada MCP HTTP Server running on port ${PORT}`);
