...
/**
 * NVV MCP HTTP Server -- Express-based Streamable HTTP MCP transport.
 *
 * Endpoints:
...
import { NvvMCPServer } from './index.js';
...
app.get('/mcp', (req, res) => {
  res.json({
    ...
    version: '1.0.0',
    name: 'NVV MCP Server',
    description: 'Skyddade naturomraden i Sverige via Naturvardsverket -- 8 verktyg',
    authentication: 'none',
    transport: 'http',
    capabilities: {
      tools: true,
    ...
    if (method === 'initialize') {
      return res.status(200).json({
        ...
        result: {
          ...
          serverInfo: { name: 'NVV MCP Server', version: '1.0.0' },
        },
      });
    }
...
<head>
  ...
  <title>NVV MCP Server -- Dokumentation</title>
  <style>
...
// -- Start ------------------------------------------------------------------
app.listen(PORT, () => {
  console.log(`NVV MCP HTTP Server running on port ${PORT}`);
