...
/**
 * Bolagsverket MCP HTTP Server — Express-based Streamable HTTP MCP transport.
 *
 * Endpoints:
...
import { BolagsverketMCPServer } from './index.js';
...
app.get('/mcp', (req, res) => {
  res.json({
    ...
    description: 'Svensk företagsinformation via Bolagsverket — 6 verktyg',
    ...
    if (method === 'initialize') {
      return res.status(200).json({
        ...
        result: {
          ...
          serverInfo: { name: 'Bolagsverket MCP Server', version: '1.0.0' },
        },
      });
    }
...
<head>
  ...
  <title>Bolagsverket MCP Server — Dokumentation</title>
  <style>
...
// ── Start ───────────────────────────────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`Bolagsverket MCP HTTP Server running on port ${PORT}`);
