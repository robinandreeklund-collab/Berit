...
/**
 * Upphandlingsdata MCP HTTP Server — Express-based Streamable HTTP MCP transport.
 *
 * Endpoints:
...
import { UpphandlingsdataMCPServer } from './index.js';
...
app.get('/mcp', (req, res) => {
  res.json({
    ...
    description: 'Svensk upphandlingsdata — 7 verktyg',
    authentication: 'none',
    transport: 'http',
    capabilities: {
      tools: true,
    ...
    if (method === 'initialize') {
      return res.status(200).json({
        ...
        result: {
          ...
          serverInfo: { name: 'Upphandlingsdata MCP Server', version: '1.0.0' },
        },
      });
    }
...
<head>
  ...
  <title>Upphandlingsdata MCP Server — Dokumentation</title>
  <style>
...
// ── Start ───────────────────────────────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`Upphandlingsdata MCP HTTP Server running on port ${PORT}`);
