...
/**
 * Visit Sweden MCP HTTP Server — Express-based Streamable HTTP MCP transport.
 *
 * Endpoints:
...
import { VisitSwedenMCPServer } from './index.js';
...
app.get('/mcp', (req, res) => {
  res.json({
    protocol: 'mcp',
    version: '1.0.0',
    name: 'Visit Sweden MCP Server',
    description: 'Svensk turistinformation — 4 verktyg',
    authentication: 'none',
    transport: 'http',
    capabilities: {
      tools: true,
    ...
    if (method === 'initialize') {
      return res.status(200).json({
        ...
        result: {
          ...
          serverInfo: { name: 'Visit Sweden MCP Server', version: '1.0.0' },
        },
      });
    }
...
<head>
  ...
  <title>Visit Sweden MCP Server — Dokumentation</title>
  <style>
...
app.listen(PORT, () => {
  console.log(`Visit Sweden MCP HTTP Server running on port ${PORT}`);
