...
/**
 * OECD MCP HTTP Server — Express-based Streamable HTTP MCP transport.
 *
 * Endpoints:
...
import { OECDMCPServer } from './index.js';
...
app.get('/mcp', (req, res) => {
  res.json({
    ...
    description: 'Internationell statistik via OECD SDMX API — 9 verktyg',
    authentication: 'none',
    transport: 'http',
    capabilities: {
      tools: true,
    ...
    if (method === 'initialize') {
      return res.status(200).json({
        ...
        result: {
          ...
          serverInfo: { name: 'OECD MCP Server', version: '1.0.0' },
        },
      });
    }
...
<head>
  ...
  <title>OECD MCP Server — Dokumentation</title>
  <style>
...
// ── Start ───────────────────────────────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`OECD MCP HTTP Server running on port ${PORT}`);
